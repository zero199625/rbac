  ...
  <div>
    <el-form :model="form" :rules="formRules" ref="formRef" label-width="100px">
      <el-form-item label="上级菜单">
        <!--
            v-model：绑定数据 与普通表单或表格差不多
            normalizer：对树形结构数据进行操作，绑定的是一个方法
        -->
        <treeselect
      ...
      <el-button type="primary" align="center" @click="doUpdate">更新</el-button>
  ...
  watch: {
    activeObj: {
      ...
      handler: function (newVal,oldVal){  // newVal=parentObj
        /* 当父组件传过来的是对象时，就用JSON转换，如果传过来的是一个参数，就从数据库中进行查找 */
        // 将父组件传过来的参数(JSON类型)转换为普通字符串，再将普通字符串转换为JSON字符串，最后赋值给表单，避免了假修改
  ...
  methods: {
    ...
    doUpdate(){
      this.$refs["formRef"].validate(flag=>{
        if (flag) {
          sysMenuApi.update(this.form).then(rs=>{
